...
def generate_image_grid(
    ...
    # load the image
    # image_path = 'out/0001_256.png'
    image_path = 'out/000014.png'
    ...
    for i, (t_cur, t_next) in tqdm.tqdm(list(enumerate(zip(t_steps[:-1], t_steps[1:]))), unit='step'): # 0, ..., N-1
        ...
        # assert not torch.any(torch.isnan(x_noised)), f'step {i}'
        ...
        # norm_grad[torch.isnan(norm_grad)] = 0.0
    ...
    PIL.Image.fromarray(image, 'RGB').save(f"out/div2k/{num_steps}.png")
    print('Done.')
...
def main():
    ...
    for step in [100]: #, 1000]:
        # generate_image_grid(f'./models/edm-ffhq-64x64-uncond-ve.pkl', f'darcy-results-{step}.png', num_steps=step, eta_in=0.1)
        # generate_image_grid(f'./models/edm-imagenet-64x64-cond-adm.pkl', f'darcy-results-{step}.png', num_steps=step, eta_in=0.1)
        generate_image_grid(f'./models/edm-imagenet-64x64-cond-adm.pkl', f'darcy-results-{step}.png', num_steps=step, eta_in=40.)
